...
def training_function(config, args):
    ...
    output_dir = Path("outputs/accelerate", time.strftime("%Y%m%d-%H%M%S"))
    ...
    fixed_noise = torch.randn(64, args.nz, 1, 1, device=accelerator.device)
    ...
    # Set up models, optimizers, and dataloaders
    discriminator, generator, optimizer_d, optimizer_g, dataloader = accelerator.prepare(
        discriminator, generator, optimizer_d, optimizer_g, dataloader
    )
    ...
    for epoch in range(args.num_epochs):
        for i, (data, _) in enumerate(dataloader):
            ...
            real = data
            bs = real.size(0)
            label = torch.full((bs,), real_label, dtype=torch.float, device=accelerator.device)
            ...
            noise = torch.randn(bs, args.nz, 1, 1, device=accelerator.device)
